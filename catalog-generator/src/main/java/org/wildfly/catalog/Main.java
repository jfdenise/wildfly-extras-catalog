...
public class Main {
    ...
    private static final String VALIDATE_PROP = "validateOnly";
    private static final String GIT_BRANCH_PROP = "git_branch";
    private static final String GIT_FORK_PROP = "git_fork";
    private static final String GIT_PROJECT_NAME_PROP = "git_project_name";

    private static final String REPLACE_WILDFLY_VERSION = "###REPLACE_WILDFLY_VERSION###";
    private static final String REPLACE_GIT_BRANCH = "###REPLACE_GIT_BRANCH###";
    private static final String REPLACE_GIT_FORK = "###REPLACE_GIT_FORK###";
    private static final String REPLACE_GIT_PROJECT_NAME = "###REPLACE_GIT_PROJECT_NAME###";
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean validateOnly = Boolean.getBoolean(VALIDATE_PROP);
        Path rootDirectory = validateOnly ? Files.createTempDirectory("wildfly-catalog-validation") : Paths.get("../docs");
        try {
            ...
            JsonNode node = mapper.readTree(String.join("", patchFile("wildfly-catalog-metadata.json", wildflyVersion)));
            ...
            if (!validateOnly) {
                ...
                Files.write(viewer, patchFile("wildfly-catalog-viewer.html", wildflyVersion));
    ...
    private static List<String> patchFile(String resource, String wildflyVersion) throws Exception {
        String fork = System.getProperty(GIT_FORK_PROP, "wildfly-extras");
        String branch = System.getProperty(GIT_BRANCH_PROP, "main");
        String projectName = System.getProperty(GIT_PROJECT_NAME_PROP, "wildfly-catalog");
            ...
            try (InputStreamReader inputStreamReader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
                try (BufferedReader bufferedReader = new BufferedReader(inputStreamReader)) {
                    ...
                    for (String line : lines) {
                        ...
                        if (line.contains(REPLACE_GIT_BRANCH)) {
                            line = line.replace(REPLACE_GIT_BRANCH, branch);
                        }
                        if (line.contains(REPLACE_GIT_FORK)) {
                            line = line.replace(REPLACE_GIT_FORK, fork);
                        }
                        if (line.contains(REPLACE_GIT_PROJECT_NAME)) {
                            line = line.replace(REPLACE_GIT_PROJECT_NAME, projectName);
                        }
                        targetLines.add(line);
                    }
                    return targetLines;
